...
async def response(request: Request, background_tasks: BackgroundTasks):
    ...
    is_success = await telegram_service.respond(json_request, background_tasks=background_tasks)
    if is_success:
        response = Response(content="Send telegram message successfully", status_code=200)
    else:
