...
async def respond(request: Dict, background_tasks: BackgroundTasks):
    is_success = False
    telegram_chat_collection = customer_db['test_telegram_chat']
    ...
    try:
        ...
        if chat:
            ...
            background_tasks.add_task(func=telegram_chat_collection.update_one, filter={"_id": chat['_id']}, update={"$set": {"messages": message_items}})

        else:
            customer_chat = CustomerChatItem(messages=[user_message, ai_message], userId=str(user_id), id=str(chat_id))
            # inserted_chat = telegram_chat_collection.insert_one(document=customer_chat)
            background_tasks.add_task(telegram_chat_collection.insert_one, document=customer_chat.model_dump())
