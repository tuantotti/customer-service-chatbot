...
def run(input_dir: str = None, method_type: str = None, api_key: str = None) -> None:
    output_dir = "output/response/"
    # client = Client()
    dataset = read_dataset(input_dir=input_dir)
    # dataset = dataset.iloc[:10, :]

    template = """Bạn là chuyên gia về chăm sóc khác hàng. Công việc của bạn là tạo ra dữ liệu hội thoại thực tế bao gồm 20 cặp câu hỏi, câu trả lời giữa khách hàng và nhân viên chăm sóc khách hàng bằng tiếng Việt về thắc mắc của người dùng có thể có về chương trình khuyến mãi sản phẩm.Giọng điệu trong câu trả lời của nhân viên chăm sóc khác hàng phải trang trọng, ấm áp và lịch sự.Mỗi cặp câu hỏi, câu trả lời phải ngăn cách nhau bởi chuỗi "************"; giữa câu hỏi và câu trả lời ngăn cách bởi chuỗi "******".20 câu hỏi được tạo ra phải chứa tên chương trình khuyến mãi, không sử dụng tên thay thế nào khác. 20 câu trả lời được tạo ra cần chèn LINK KHUYẾN MÃI nếu bạn thấy hợp lý.
